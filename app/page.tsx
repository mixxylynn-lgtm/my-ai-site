...
export default function Home() {
  ...
  const generate = async () => {
    ...
    const prompt = tab === "listing"
      ? `You are an expert marketplace seller. Write a high-converting product listing for ${platform} for this item: "${item}". Include: 1) An optimized title (keyword-rich, platform appropriate), 2) A compelling description (3-4 paragraphs), 3) Suggested keywords/tags. Format it clearly with labels.`
      : `You are an expert reseller with deep knowledge of marketplace pricing. For this item: "${item}" on ${platform} — provide: 1) A suggested selling price range, 2) How condition affects price, 3) Tips to get the highest price, 4) Whether it is worth selling. Be specific and practical.`;
    ...
    <main style={{minHeight:"100vh",background:"#0a0a0a",color:"white",fontFamily:"system-ui,sans-serif"}}>
      ...
      <section id="tools" style={{maxWidth:"620px",margin:"0 auto",padding:"0 24px 80px"}}>
        ...
        <div style={{display:"flex",gap:"8px",marginBottom:"16px",background:"#111",padding:"5px",borderRadius:"10px",border:"1px solid #1f1f1f"}}>
          <button onClick={()=>{setTab("listing");setResult("");}} style={{flex:1,padding:"11px",borderRadius:"7px",border:"none",cursor:"pointer",fontWeight:"700",fontSize:"14px",background:tab==="listing"?"#22d3ee":"transparent",color:tab==="listing"?"black":"#555"}}>
            ✍️ Write My Listing
          </button>
          <button onClick={()=>{setTab("price");setResult("");}} style={{flex:1,padding:"11px",borderRadius:"7px",border:"none",cursor:"pointer",fontWeight:"700",fontSize:"14px",background:tab==="price"?"#22d3ee":"transparent",color:tab==="price"?"black":"#555"}}>
            💰 Price My Item
          </button>
        </div>
        ...
        <div style={{background:"#111",border:"1px solid #22d3ee",borderRadius:"16px",padding:"32px"}}>
          ...
          <ul style={{listStyle:"none",padding:0,marginBottom:"24px",textAlign:"left"}}>
            {[
              ...
              "Works for eBay, Etsy, Depop & more",
              "AI price estimator",
